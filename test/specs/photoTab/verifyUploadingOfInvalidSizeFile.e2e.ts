...
const filePath = path.join(__dirname, '../../images/SamplePNGImage_30mbmb.png');

describe('id:C405 - Verify uploading of invalid size file', () => {
    before(async () => {
    ...
    it('1. Upload file with invalid size (more than 20Mb). Check notification pop-up appears.', async () => {
        ...
        await expect(profilePage.popUpWarningText).toHaveText(/Ви не можете завантажити файл більше 20 МВ./);
    });
    ...
    it('3. Upload file with invalid size (more than 20Mb). Cclick on [saveBtn] button.', async () => {
        const input = profilePage.imageInput;
        const remoteFilePath = await browser.uploadFile(filePath);
        await input.setValue(remoteFilePath); 
    ...
    it('4. Upload file with invalid size (more than 20Mb). Click outside of pop-up window.', async () => {
        const input = profilePage.imageInput;
        const remoteFilePath = await browser.uploadFile(filePath);
        await input.setValue(remoteFilePath); 
